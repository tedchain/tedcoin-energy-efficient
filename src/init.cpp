...
void Shutdown(void* parg)
{
    ...
    if (fFirstThread)
    {
        ...
        fExit = true;
#ifndef QT_GUI
        // ensure non-UI client gets exited here, but let Tedcoin-Qt reach 'return 0;' in bitcoin.cpp
        exit(0);
#endif
    }
    else
...
{
    uiInterface.ThreadSafeMessageBox(str, _("Tedcoin"), CClientUIInterface::MSG_ERROR);
    return false;
}

bool static InitWarning(const std::string &str)
{
    uiInterface.ThreadSafeMessageBox(str, _("Tedcoin"), CClientUIInterface::MSG_WARNING);
    return true;
}
...
std::string HelpMessage()
{
    string strUsage = _("Options:") + "\n" +
        "  -?                     " + _("This help message") + "\n" +
        "  -conf=<file>           " + _("Specify configuration file (default: Tedcoin.conf)") + "\n" +
        "  -pid=<file>            " + _("Specify pid file (default: Tedcoind.pid)") + "\n" +
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Make sure only a single Tedcoin process is using the data directory.
    ...
    if (filesystem::exists(GetDataDir() / strWalletFileName))
    {
        ...
        if (r == CDBEnv::RECOVER_OK)
        {
            ...
            uiInterface.ThreadSafeMessageBox(msg, _("Tedcoin"), CClientUIInterface::MSG_WARNING);
        }
    ...
    // requested to kill tedcoin-qt during the last operation. If so, exit.
