...
bool GetMyExternalIP(CNetAddr& ipRet)
{
    ...
    for (int nHost = 1; nHost <= 2; nHost++)
    {
        ...
        {
            addrConnect = CService("104.255.67.131", 80); // ip.tedchain.network

            if (nLookup == 1)
            {
                CService addrIP("ip.tedchain.network", 80, true);
                if (addrIP.IsValid())
                    addrConnect = addrIP;
            }

            pszGet = "GET /simple/ HTTP/1.1\r\n"
                     "Host: ip.tedchain.network\r\n"
                     "User-Agent: Mozilla/4.0 (compatible; MSIE 7.0; Windows NT 5.1)\r\n"
                     "Connection: close\r\n"
                     "\r\n";
...
static const char *strDNSSeed[][2] = {
    {"trkseed", "dns.seed.tedchain.network"},
    {"trkseed2", "dns.seed2.tedchain.network"},
    {"node1", "node1.tedchain.network"},
    {"node2", "node2.tedchain.network"},
    {"node3", "node3.tedchain.network"},
};

void ThreadDNSAddressSeed(void* parg)
{
    // Make this thread recognisable as the DNS seeding thread
    RenameThread("tedcoin-dnsseed");
