	...
	blockBrowser(0)
{
    updateStyle();
    resize(860, 600);
    setWindowTitle(tr("Tedcoin") + " - " + tr("Wallet ") + QString::fromStdString(FormatFullVersion()));
#ifndef Q_OS_MAC
...
void BitcoinGUI::createActions()
{
    ...
    chatAction->setToolTip(tr("TRK IRC chat, visit Tedcoin on the Web"));
...
void BitcoinGUI::setClientModel(ClientModel *clientModel)
{
    ...
    if(clientModel)
    {
        ...
        if(clientModel->isTestNet())
        {
            ...
            if(trayIcon)
            {
                trayIcon->setToolTip(tr("Tedcoin client") + QString(" ") + tr("[testnet]"));
...
void BitcoinGUI::createTrayIcon()
{
    ...
    trayIcon->setToolTip(tr("Tedcoin client"));
...
void BitcoinGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
        ...
        if (nValidUrisFound)
            ...
            message(tr("URI handling"), tr("URI can not be parsed! This can be caused by an invalid Tedcoin address or malformed URI parameters."), 
...
void BitcoinGUI::updateStyle()
{
    ...
    QString qssPath = QString::fromStdString( GetDataDir().string() ) + "/Tedcoin.qss";
